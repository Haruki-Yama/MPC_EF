...
def simulate(
    ...
    **options
    ...
    """
    #     １次元データ補完(interp1d)
    spl_δ = interp1d(time_list, δ_list, "linear", fill_value="extrapolate")
    spl_npm = interp1d(time_list, npm_list, "linear", fill_value="extrapolate")
    ...
    def mmg_3dof_eom_solve_ivp(t, X):
        ...
        u, v, r, x, y, ψ, δ, npm , X_F, Y_F, N_F= X
        ...
        dX_F = derivative(spl_X_F, t)
        dY_F = derivative(spl_Y_F, t)
        dN_F = derivative(spl_N_F, t)
        
        
        # return [d_u, d_v, d_r, d_x, d_y, d_ψ, d_δ, d_npm] 
        return [d_u, d_v, d_r, d_x, d_y, d_ψ, d_δ, d_npm, dX_F, dY_F, dN_F] 
        

    sol = solve_ivp(
        mmg_3dof_eom_solve_ivp,
        [time_list[0], time_list[-1]],
        [u0, v0, r0, x0, y0, ψ0, δ_list[0], npm_list[0], X_F_list[0], Y_F_list[0], N_F_list[0]],
        dense_output=True,
        method=method,
        t_eval=t_eval,
        events=events,
